...
// /verify carries the page locale ("en"), /confirm the browser's navigator
// .language ("en-US"). The site sends two different values here, so we do too.
const DEFAULT_CONFIRM_LOCALE = 'en-US';

const apiBase = (config) => config.api_base || DEFAULT_API_BASE;
const locale = (config) => config.locale || DEFAULT_LOCALE;
const confirmLocale = (config) => config.confirm_locale || DEFAULT_CONFIRM_LOCALE;
const consentEmail = (config) => config.consent_email !== false;
const consentMessaging = (config) => config.consent_messaging !== false;
 ...
 * up with preferencePollIds[i]). Non-city polls come from polls.fixed, where an
 * entry may either pin one answer (`pollAnswerIds`, e.g. the album quiz, which
 * has a correct answer) or draw one per task (`randomAnswerIds`, e.g. the travel
 * poll, so not every signup answers identically).
...
function buildPollAnswers(config) {
  ...
  const journeyPollAnswers = (config.polls.fixed || []).map(entry => ({
    pollId: entry.pollId,
    pollAnswerIds: entry.randomAnswerIds?.length
      ? [pickOne(entry.randomAnswerIds)]
      : entry.pollAnswerIds,
  }));
  rest.forEach((city, i) => {
...
async function confirm(session, config, { token, captcha, location, ip, profile, polls }) {
  const res = await session.postJson(
    apiBase(config) + PATHS.confirm,
    {
      ...
      consentMessaging: consentMessaging(config),
